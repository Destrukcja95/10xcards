...
import { generationSessionsQuerySchema, sessionIdParamSchema, updateSessionSchema } from "./generation-sessions.schema";
...
describe("generation-sessions.schema", () => {
  ...
  describe("generationSessionsQuerySchema", () => {
    ...
    it("should reject page less than 1", () => {
      expect(() => generationSessionsQuerySchema.parse({ page: "0" })).toThrow("Page must be at least 1");
    });
    ...
    it("should reject limit greater than 100", () => {
      expect(() => generationSessionsQuerySchema.parse({ limit: "101" })).toThrow("Limit must be between 1 and 100");
    });
  ...
  describe("sessionIdParamSchema", () => {
    ...
    it("should reject invalid UUID format", () => {
      expect(() => sessionIdParamSchema.parse({ id: "invalid-uuid" })).toThrow("Invalid session ID format");
    });
    ...
    it("should reject too short UUID", () => {
      expect(() => sessionIdParamSchema.parse({ id: "550e8400-e29b-41d4" })).toThrow("Invalid session ID format");
    });

    it("should reject UUID with wrong separator", () => {
      expect(() => sessionIdParamSchema.parse({ id: "550e8400_e29b_41d4_a716_446655440000" })).toThrow(
  ...
  describe("updateSessionSchema", () => {
    ...
    it("should reject negative accepted_count", () => {
      expect(() => updateSessionSchema.parse({ accepted_count: -1 })).toThrow("accepted_count must be non-negative");
    });

    it("should reject non-integer accepted_count", () => {
      expect(() => updateSessionSchema.parse({ accepted_count: 5.5 })).toThrow("accepted_count must be an integer");
    });
