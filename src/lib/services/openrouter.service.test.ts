...
describe('parseFlashcards logic', () => {
  ...
  it('should throw PARSE_ERROR when response is not an array', () => {
    // Content has array brackets but contains object instead of array elements
    const content = '[{"key": "not a flashcard array but wrapped in brackets"}]';
    // This will parse but fail validation - let's test the actual case
